...
class AttendanceSystemGUI:
    ...
    def show_monthly_summary(self):
        ...
        def export_monthly():
            ...
            # エクスポート実行
            result = self.export_monthly_summary_to_csv(month_str)
        ...
        # エクスポートボタン
        tk.Button(input_frame, text="エクスポート", command=export_monthly,
                 font=("Arial", 12), bg="green", fg="white").pack(side=tk.LEFT, padx=5)
    ...
    def export_monthly_summary_to_csv(self, month_str):
        """月次集計CSVエクスポート"""
        try:
            conn = sqlite3.connect(self.db_path)
            cursor = conn.cursor()
            
            # 月の全レコードを取得（講師名と日付でグループ化）
            cursor.execute('''
            ...
            # CSVファイル名を生成（実行元フォルダに保存）
            csv_filename = f"出退勤記録_{month_str}.csv"
            ...
            with open(csv_filename, 'w', newline='', encoding='utf-8-sig') as csvfile:
                writer = csv.writer(csvfile)
                writer.writerow(['講師名', '出勤回数', '単位数（4コマ）', '時間数（3.25h）'])
                
                for name in sorted(instructor_attendance.keys()):
                    attendance_count = instructor_attendance[name]
                    units = attendance_count * 4
                    hours = attendance_count * 3.25
                    writer.writerow([name, attendance_count, units, hours])
            
            # 統計情報
            total_instructors = len(instructor_attendance)
            total_days = sum(instructor_attendance.values())
            
            result = f"=== 月次集計エクスポート完了 ===\n\n"
            result += f"ファイル名: {csv_filename}\n"
            result += f"対象月: {month_str}\n\n"
            result += f"=== 集計結果 ===\n"
            result += f"出勤した講師数: {total_instructors}人\n"
            result += f"総出勤日数: {total_days}日\n\n"
            result += f"=== 講師別出勤回数 ===\n"
            
            for name in sorted(instructor_attendance.keys()):
                result += f"{name}: {instructor_attendance[name]}回\n"
